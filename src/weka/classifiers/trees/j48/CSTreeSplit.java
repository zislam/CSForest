...
/**
 * Class implementing a CSTree-type split on an attribute.
 *
 * @author modified by Michael Furner (mfurner@csu.edu.au) (originally by Eibe
 * Frank (eibe@cs.waikato.ac.nz))
 */
public class CSTreeSplit extends ClassifierSplitModel {
     ...
     */
    protected static CSTreeCostSplitCrit m_costCrit;
    ...
    /**
     * The total expected cost prior to splitting
     */
    protected double m_totalExpectedCost;
    
    /**
     * Initializes the split model.
     */
    public CSTreeSplit(int attIndex, int minNoObj, double sumOfWeights,
            CostMatrix costMatrix, double totalExpected) {

        m_totalExpectedCost = totalExpected;
        m_costCrit = new CSTreeCostSplitCrit(costMatrix);
    ...
    /**
     * Creates a CSTree-type split on the given data. Assumes that none of the
    ...
    public final double classProb(int classIndex, Instance instance, int theSubset)
        ...
        } else if (Utils.gr(m_distribution.perBag(theSubset), 0)) {
    ...
    private void handleEnumeratedAttribute(Instances trainInstances)
        ...
        if (m_distribution.check(m_minNoObj)) {
            ...
            m_cost = m_costCrit.splitCritValue(m_distribution, trainInstances.classAttribute());
        }
    }
    ...
    private void handleNumericAttribute(Instances trainInstances)
        ...
        // subset.
        /**
         * ****** MODIFIED BY MICHAEL: ONLY USE m_minNoObj minSplit = 0.1 *
         * (m_distribution.total()) / (trainInstances.numClasses()); if
         * (Utils.smOrEq(minSplit, m_minNoObj)) { minSplit = m_minNoObj; } else
         * if (Utils.gr(minSplit, 25)) { minSplit = 25; } 
    **********
         */
        minSplit = m_minNoObj;
        ...
        while (next < firstMiss) {
            ...
            if (trainInstances.instance(next - 1).value(m_attIndex) + 1e-5 < trainInstances
                ...
                if (Utils.grOrEq(m_distribution.perBag(0), minSplit)
                    ...
                    currentCost = m_costCrit.splitCritValue(m_distribution, trainInstances.classAttribute());

                    if (Utils.smOrEq(currentCost, m_cost)) {
                        m_cost = currentCost;
                        splitIndex = next - 1;
        ...
        if (m_index == 0 || validModelFound != true) {
            return;
        }

        //if doesn't satisfy E - E_{A_i} >= 0
        if (!Utils.grOrEq(m_totalExpectedCost - m_cost, 0)) {
            return;
        }
        ...
        for (int k = 0; k < m_distribution.actualNumBags(); k++) {
            if (m_distribution.perBag(k) == 0) {
                m_numSubsets = 0;
            }
        }
    ...
    /**
     * Gets class costs for instance.
     *
     * @exception Exception if something goes wrong
     */
    public double classCost(int classIndex, Instance instance, int theSubset)
            throws Exception {
   ...
   * @exception Exception if something goes wrong
   */
    public final String dumpLabel(int index,Instances data, boolean dumpLabel) throws Exception {
